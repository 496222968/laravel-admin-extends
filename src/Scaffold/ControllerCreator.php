...
class ControllerCreator
{
    ...
    {
        $this->name = $name;
        $this->lang = request()->get('table_name');
...
//            $rows[] = "\$form->text('{$field['name']}', trans('{$lang}.fields.{$field['name']}'));\n";
            $rows[] = "\$form->{$field['form']}('{$field['name']}', trans('{$lang}.fields.{$field['name']}'));\n";
        }
    ...
    public function generateShowField($fields = [])
    {
        ...
        $lang = $this->lang;
        foreach ($fields as $field) {
            $rows[] = "\$show->{$field['name']}(trans('{$lang}.fields.{$field['name']}'));\n";
        }
    ...
    public function generateGridField($fields = [])
    {
        ...
        $lang = $this->lang;
        foreach ($fields as $field) {
            $rows[] = "\$grid->{$field['name']}(trans('{$lang}.fields.{$field['name']}'));\n";
        }
